...
package iee.wirelesscharge.Dialogue;
...
public class DeviceListActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(this.checkPermission(
                Manifest.permission.WRITE_EXTERNAL_STORAGE, Binder.getCallingPid(),Binder.getCallingUid()) != PackageManager.PERMISSION_GRANTED) {
        ...
        //判断是否有权限
        if (this.checkPermission(
                Manifest.permission.ACCESS_COARSE_LOCATION, Binder.getCallingPid(),Binder.getCallingUid()) != PackageManager.PERMISSION_GRANTED) {
            //请求权限
...
//        } else {
        ...
        if (getIntent().getBooleanExtra("doDiscover",false)){
            doDiscovery();
        }
    }
	...
	private void doDiscovery() {
        ...
        // Editted : Only show devices exist
        // If there are paired devices, add each one to the ArrayAdapter
//        if (pairedDevices.size() > 0) {
//            for (BluetoothDevice device : pairedDevices) {
//                mPairedDevicesArrayAdapter.add(device.getName() + "\n" + device.getAddress());
    ...
    private OnItemClickListener mDeviceClickListener = new OnItemClickListener() {
        public void onItemClick(AdapterView<?> av, View v, int arg2, long arg3) {
	        ...
	        if(!((TextView) v).getText().toString().equals(strNoFound)) {
                ...
                String[] parts = info.split(" RSSI:|dBm\n");
//	            String address = info.substring(info.length() - 17);
	            
	            // Create the result Intent and include the MAC address
	            Intent intent = new Intent();
	            intent.putExtra(DeviceListActivity.EXTRA_DEVICE_NAME, parts[0]);
                intent.putExtra(DeviceListActivity.EXTRA_DEVICE_RSSI, parts[1]);
                intent.putExtra(DeviceListActivity.EXTRA_DEVICE_ADDRESS, parts[2]);
...
//                if (device.getBondState() != BluetoothDevice.BOND_BONDED) {
                	...
                	mPairedDevicesArrayAdapter.add(device.getName() + " RSSI:" + rssi+ "dBm\n" + device.getAddress());
//                }
